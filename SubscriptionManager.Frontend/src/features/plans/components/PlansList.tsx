...
import { Plus, Edit2, Trash2, ListTodo, AlertCircle, Search, Star, X, Loader2 } from 'lucide-react';
...
export const PlansList = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      const errorMsg = err.response?.data?.detail || err.response?.data || 'Failed to delete plan. It might be in use.';
          ...
          <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden border border-[#E3E8EE] animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-[#E3E8EE]">
              <h3 className="text-lg font-bold text-rose-600 flex items-center gap-2">
                <Trash2 className="w-5 h-5" /> Delete Plan
              </h3>
            </div>
